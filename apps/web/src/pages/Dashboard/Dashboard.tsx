...
export default function Dashboard() {
  ...
  const { startDate: prevStartDate, endDate: prevEndDate } = useMemo(() => {
    return getPreviousPeriodRange(currentMonthDate, startDayOfMonth);
  }, [currentMonthDate, startDayOfMonth]);

  const periodLabel = startDayOfMonth === 1 ? monthName : rangeLabel;
  const isCalendarMonth = startDayOfMonth === 1;
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground text-xs font-medium pl-1">
            ...
            {!isCalendarMonth && ` (Cycle start: ${startDayOfMonth})`}
          </p>
        </div>
